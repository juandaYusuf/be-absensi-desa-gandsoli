...
async def getQrCodeData(data_option : str):
    ...
    try:
        ...
        if len(response_qrcode_in) == 0 or len(response_qrcode_out) == 0:
            ...
            insert_data_qrcode_in = conn.execute(qrcode_data_in.insert().values(tmstmp = timestamp_string, status = "in"))
            insert_data_qrcode_out = conn.execute(qrcode_data_out.insert().values(tmstmp = timestamp_string, status = "out"))
            if insert_data_qrcode_in.rowcount > 0 and insert_data_qrcode_out.rowcount > 0 :
                ...
                if result_data_qrcode_in and result_data_qrcode_out:
                    ...
                    insert_qrcode_data = conn.execute(qrcode_data.insert().values(qrcode_in_id = result_data_qrcode_in.id, qrcode_out_id = result_data_qrcode_out.id, tmstmp = timestamp_string))
            ...
            if get_last_record_date_from_db_qrcode_in == date_now_for_compare:
                ...
                if insert_data_qrcode_in.rowcount > 0 and insert_data_qrcode_out.rowcount > 0 :
                    ...
                    insert_qrcode_data = conn.execute(qrcode_data.insert().values(qrcode_in_id = qrcode_in_after_insert.id, qrcode_out_id = qrcode_out_after_insert.id, tmstmp = timestamp_string))
                    if insert_qrcode_data.rowcount > 0:
